...
async function scanRepo(owner, repoName) {
  ...
  try {
    ...
    for (const pr of prs) {
      ...
      let totalChanges = 0, changesSensitive = false;
      for (const f of files) { totalChanges += f.changes || 0; for (const sd of SENSITIVE_DIRS) if (f.filename.startsWith(sd + '/')) changesSensitive = true; }
      const noReviewers = (pr.requested_reviewers || []).length === 0;
      if (totalChanges > 200 || noReviewers || changesSensitive) {
        const severity = (totalChanges > 500 || changesSensitive || noReviewers) ? 'high' : 'medium';
        addFinding({ type: 'risky_pr', severity, resource: `${owner}/${repoName} PR#${pr.number}`, description: `PR ${pr.number} title="${pr.title}" changes=${totalChanges} sensitive=${changesSensitive} reviewers=${(pr.requested_reviewers||[]).length}`, metadata: { pr_number: pr.number, changes: totalChanges } });
      }
    }
